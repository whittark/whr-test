...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:import href="plugin:com.oxygenxml.webhelp.responsive:xsl/util/relpath_util.xsl"/>

    <xsl:import href="plugin:com.oxygenxml.webhelp.responsive:xsl/util/fixupNS.xsl"/>
            ...
            <xsl:choose>
                <xsl:when test="self::*[contains(@class, ' bookmap/chapter ')]">
                    ...
                    <xsl:call-template name="getString">
                        ...
                        <xsl:text> </xsl:text>
                        <xsl:value-of select="count(preceding-sibling::*[contains(@class, ' bookmap/chapter ')][not(contains(@processing-role, 'resource-only'))][not(contains(@class, ' bookmap/frontmatter '))][not(contains(@class, ' bookmap/backmatter '))][not(contains(@toc, 'no'))])+1"/>
                    <xsl:text> </xsl:text>
                    </xsl:if>
                </xsl:when>
            </xsl:choose>
