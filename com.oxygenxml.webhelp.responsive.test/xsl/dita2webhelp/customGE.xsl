...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="FTR"/>
	<xsl:param name="PRM_SUBHEADERNUM"/>
	...
	<!-- GEHC-144 -->
        <!-- Rally ID DE299 -->
	<!-- 2018-04-13: add topic-section numbering to nested topics; section numbering only for root topic -->

	<!-- NESTED TOPIC TITLES (sensitive to nesting depth, but are still processed for contained markup) -->
	<!-- 1st level - topic/title -->
	<!-- Condensed topic title into single template without priorities; use $headinglevel to set heading.
     If desired, somebody could pass in the value to manually set the heading level -->
	<xsl:template match="*[contains(@class, ' topic/topic ')][parent::*[contains(@class, ' topic/topic ')]]/*[contains(@class, ' topic/title ')]">
		...
		<xsl:element name="h{$headinglevel}">
			...
			<xsl:if test="$PRM_SUBHEADERNUM='yes'">
				<xsl:choose>
					<xsl:when test="parent::*[contains(@class, ' topic/topic ')] and ancestor::*[contains(@class, ' topic/topic ')]">
						<!-- nested topic but not root; root topic does not receive a subhead number -->
					
							<xsl:number format="1" 
								count="*[contains(@class, ' topic/topic ')][parent::*[contains(@class, ' topic/topic ')]]" level="multiple" grouping-separator="."/> 
						<xsl:text> </xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:message>Issue with ge.add.subheader.prefix parameter</xsl:message>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:if>
			<xsl:apply-templates/>
			...
			<xsl:if test="$PRM_SUBHEADERNUM='yes'">
				<xsl:choose>
					...
					<xsl:when test="ancestor::*[contains(@class, ' topic/topic ')]">
						<!-- section in a nested topic but not root; root not included in subhead number -->
						
						<xsl:number format="1" 
							count="*[contains(@class, ' topic/topic ')][parent::*[contains(@class, ' topic/topic ')]]|*[contains(@class, ' topic/section ')]" level="multiple" grouping-separator="."/>
						<xsl:text> </xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:message>Issue with ge.add.subheader.prefix parameter</xsl:message>
...
-->
	...
	<xsl:template match="*[contains(@class, ' topic/link ')]" priority="1000"
